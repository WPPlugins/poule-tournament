...
class matches {
    ...
    public function home(){
        ...
        $selectphase = get_term_by('slug', $phase, $taxonomyname);
        
        $groups = array();
        foreach($wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}poule_matches_groups WHERE phase='%s' ORDER BY group_name ASC", $selectphase->term_id)) as $group){
            $matches = array();
            
            foreach ($wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}poule_matches WHERE group_id='%s'",$group->id)) as $match) {
...
//                wp_safe_redirect( '?post_type=country&page=matches&function=edit&phase=groep&groupid='.$groupsid,200); 
    ...
    public function auto(){
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if($poule_validation->counterrors['group'] == 0){
                $place = array();
                $groups_new = array();
                foreach ($wpdb->get_results($wpdb->prepare("SELECT id FROM {$wpdb->prefix}poule_matches_groups WHERE phase='%s'",$phase_id)) as $group_number => $group) {
                    $groups_new[$group->id] = array('row' => $group_number);
                    $place[$group->id] = array();

                    foreach ($wpdb->get_results($wpdb->prepare("SELECT id,country_1,country_2,score FROM {$wpdb->prefix}poule_matches WHERE group_id='%s'",$group['id'])) as $value) {
                ...
                foreach ($groups_new as $group_id => $value) {
                    foreach ($value as $match) {
                        if ($phase_id == $phases[0]->term_id) {
                            $place = $this->CalculateScore($group_id, $place, $match, TRUE);
							$place = $this->CalculateScore($group_id, $place, $match, FALSE);
                        } else {
							$place = $this->CalculateScoreKnockout($match, $place, $group_id);
                        }
                    }
                }
                ...
                foreach ($poule_validation->input['group'] as $key => $group) {
                    ...
                    $wpdb->insert($wpdb->prefix."poule_matches_groups", $insert_group);
        ...
        for($g = 1; $g <= $term_meta->groups; $g++){
        	...
        	$data = array();
        	for($i = 1; $i <= $term_meta->matches_per_group; $i++){
        		$data_error = (!isset($errors[$g]['matches'][$i])) ? array() : $errors[$g]['matches'][$i];
				$data_input = (!isset($input[$g]['matches'][$i])) ? array() : $input[$g]['matches'][$i];
			...
			$data_error = (!isset($errors[$g]['name'])) ? "" : $errors[$g]['name'];
			$data_input = (!isset($input[$g]['name'])) ? "" : $input[$g]['name'];
	...
	private function CalculateScoreKnockout($match, $place, $group_id){
	...
	private function CalculateScore($group_id, $place, $match, $country1 = TRUE){
		$score = array('won' => 0, 'equal' => 0, 'lost' => 0, 'positive' => 0, 'negative' => 0);
		
		$country = ($country1) ? "country_1" : "country_2" ;
		
		if($match['score_1'] > $match['score_2']) $score['won']++;
		else if ($match['score_1'] == $match['score_2']) $score['equal']++;
		else $score['lost']++;
		
		$points = $score['won'] * 3 + $score['equal'];
		if (array_key_exists($match[$country], $place[$group_id])) {
			$place[$group_id][$match[$country]]['played']++;
			$place[$group_id][$match[$country]]['won'] += $score['won'];
			$place[$group_id][$match[$country]]['equal'] += $score['equal'];
			$place[$group_id][$match[$country]]['lost'] += $score['lost'];
			$place[$group_id][$match[$country]]['points'] += $points;
			
			if($country1){
				$place[$group_id][$match[$country]]['positive'] += $match['score_1'];
				$place[$group_id][$match[$country]]['negative'] += $match['score_2'];
			}else{
				$place[$group_id][$match[$country]]['positive'] += $match['score_2'];
				$place[$group_id][$match[$country]]['negative'] += $match['score_1'];
			}
			
			$goal_difference = $place[$group_id][$match[$country]]['positive'] - $place[$group_id][$match['country_1']]['negative'];
			$place[$group_id][$match[$country]]['goal_difference'] = $goal_difference;

			$points2 = $place[$group_id][$match[$country]]['points'] + $goal_difference / 100 + $place[$group_id][$match[$country]]['positive'] / 1000;
			$place[$group_id][$match[$country]]['points2'] = $points2;
		} else {
			$place[$group_id][$match[$country]] = array('played' => 1, 'won' => $score['won'], 'equal' => $score['equal'], 'lost' => $score['lost'], 'points' => $points, 'positive' => $match['score_1'], 'negative' => $match['score_2']);
		}
	 ...
	 * Create the drop down to select the country for the match. it's groupt by the group
	 * 
	 * @todo select zichtbaar maken bij een foute post + betere fase gebruiken voor vorgige fase
	 * @access public
	 * @author Stefan de Bruin <info@stefandebruin.eu>
	 * @package poule_tournament
	 * @since version 2.2
