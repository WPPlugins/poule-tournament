...
//			var_dump($poule_validation->counterrors['score']);
//			var_dump($poule_validation->input);
//			var_dump($poule_validation->errors);
//			var_dump($_POST['score']);
            if($poule_validation->counterrors['score'] == 0){
                foreach($poule_validation->input['score'] as $key => $match){
					...
					foreach(poule_get_phases() as $phasesql){
						...
						if($meta != null && $meta != 0 && $phase == $phasesql){
							$penalties = TRUE;
						}
					}
					...
					if($penalties && $score1 === $score2 && $score1 != 0){
        ...
        foreach($wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}poule_matches_groups WHERE phase='%s' ORDER BY group_name ASC", $term->term_id),ARRAY_A) as $rowid => $group){
			...
			$table = new create_table('score','home');
...
//		return '';
	...
	public function validate_score($value, $key = null, $row = null){
		...
		//controleer instellingen 1 keer invullen 
	...
	public function validate_penalty($value, $inputname = null, $row = null, $key = null){
		...
		$phaseurl = (isset($_GET['phase'])) ? $_GET['phase'] : poule_get_phases()[0]->slug;
